...
describe("StakedTLOS", function () {
  ...
  describe("Core", () => {
    ...
    it("Should observe yield only increases", async function () {
      const [owner, addr1, addr2] = await ethers.getSigners();

      await stlos.asset();
      await stlos.depositTLOS({value: ethers.utils.parseEther("1.0")});
      console.log("Checking balanceOf owner to == 1");
      expect(await stlos.balanceOf(owner.address)).to.equal(ethers.utils.parseEther("1.0"), "Owner address shoudl get 1 sTLOS for depositing 1 TLOS");
      ...
      console.log("Checking max withdraw of addr1 to == 1");
